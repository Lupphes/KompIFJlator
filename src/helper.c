...
int addStringToStringArray(StringArray* arr, string* str){
    ...
    if ((arr->str = realloc(arr->str,sizeof(string)*newCount)) == NULL)
        return INTERNAL_ERROR;
    callAndHandleException(strInit(&arr->str[newCount-1]));
    callAndHandleException(strCopyString(&arr->str[newCount-1],str));
